...
class Index:
    ...
    def fit(self, docs):
        ...
        for field in self.text_fields:
            # Convert each field value to a string, handling non-string and missing values, then strip
            texts = [str(doc.get(field, '') or '').strip() for doc in docs]
            
            # Skip this field if all values are empty
            if all(text == '' for text in texts):
                print(f"Warning: No valid texts found for '{field}', skipping this field.")
                continue
            
            self.text_matrices[field] = self.vectorizers[field].fit_transform(texts)

        # Collect data for keyword fields
        for doc in docs:
            for field in self.keyword_fields:
                keyword_data[field].append(doc.get(field, ''))

        # Create a DataFrame for keyword fields for easy filtering
